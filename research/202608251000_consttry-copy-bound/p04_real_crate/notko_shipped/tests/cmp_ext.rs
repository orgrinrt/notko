...
fn integer_comparisons() {
    assert!(matches!(
        1_i32.partial_cmp_maybe(&2),
        Maybe::Is(Ordering::Less)
    ));
    assert!(matches!(
        2_i32.partial_cmp_maybe(&2),
        Maybe::Is(Ordering::Equal)
    ));
    assert!(matches!(
        3_i32.partial_cmp_maybe(&2),
        Maybe::Is(Ordering::Greater)
    ));
}
...
fn equivalent_to_partial_cmp_into() {
    let pairs: [(f64, f64); 4] = [(1.0, 2.0), (2.0, 2.0), (3.0, 2.0), (f64::NAN, 1.0)];
    for (a, b) in pairs {
