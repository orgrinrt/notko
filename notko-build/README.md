...
discoverable via `#[optimize_for(<name>)]`.
...
and usable by `#[optimize_for(<name>)]`.
...
5. Emits:
   - `cargo:rustc-env=NOTKO_OPTIMISERS_PATH=$OUT_DIR/notko-optimisers` —
     the notko-macros proc-macro reads this during expansion.
   - `cargo:notko-optimiser-path=$OUT_DIR/notko-optimisers` — propagates
     this crate's accumulated optimisers to downstream dependents (only
     takes effect if the crate declares `links = "notko-optimisers-..."`).
   - `cargo:rerun-if-changed=notko-optimizers` — invalidates the build
     when optimiser files change.
...
3. `$NOTKO_OPTIMISERS_PATH/<name>.rs` — accumulated (requires
   notko-build in the consumer's build.rs).
